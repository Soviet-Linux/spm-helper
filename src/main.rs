...
fn main() {
    ...
    let dependencies =
        user_input("Enter the dependencies of the package(example \"git\", \"make\"): ".to_owned());

    let prepare_info = user_input(
        "Enter the the command to prepare the package (SPACE + ENTER if none) : ".to_owned(),
    );

    let make_info = user_input(
        "Enter the the command to make the package (SPACE + ENTER if none) : ".to_owned(),
    );

    let test_info = user_input(
        "Enter the the command to test the package (SPACE + ENTER if none) : ".to_owned(),
    );

    let install_info = user_input(
        "Enter the the command to install the package (use $BUILD_ROOT for the install dir ) : "
            .to_owned(),
    );
...
fn save_as_file(text: Value, name: String) {
    ...
    if selections[selection] == "Yes" {
        ...
        // Write a byte string.
        file.write(text_pretty.unwrap().to_owned().as_bytes())
            .unwrap();
...
fn ask_for_type() -> String {
    let selections = &["Source", "Binary", "local"];
